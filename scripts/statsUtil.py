...
def updateStats(week, season):
	...
	if num[0][0] == 0:
		...
		cur.execute("select homeScore + awayScore from Game where weekNumber=" + week + " and season=" + season + " order by gameID desc limit 1")
		MNF = cur.fetchall()
	...
	for conf in conferences:
		# grab the top 10 division winners
		winnerCount = 0
		winnerIDs = []
		currRow = [0,50000,0,0,0]
		...
		for player in leaders:
			# see if this row is ahead of the current one
			if winnerCount < 10 and (player[1] < currRow[1] or (player[1] == currRow[1] and (player[2] < currRow[2] or (player[2] == currRow[2] and ((currRow[3] == 0 and player[3] > 0) or player[4] < currRow[4]))))):
				winnerCount = winnerCount + len(winnerIDs)
				for winner in winnerIDs:
					cur.execute("update SeasonResult set firstRoundBye=if(" + str(winnerCount) + " <= 10, 'Y', 'R') where season=" + season + " and userID=" + str(winner))
				winnerIDs = [player[0]]
				currRow = player
			else:
				winnerIDs.append( player[0] )
		...
		for player in leaders:
			# see if this row is ahead of the current one
			if winnerCount < 30 and (player[1] < currRow[1] or (player[1] == currRow[1] and (player[2] < currRow[2] or (player[2] == currRow[2] and ((currRow[3] == 0 and player[3] > 0) or player[4] < currRow[4]))))):
				winnerCount = winnerCount + len(winnerIDs)
				for winner in winnerIDs:
					cur.execute("update SeasonResult set inPlayoffs=if(" + str(winnerCount) + " <= 30, 'Y', 'R') where season=" + season + " and userID=" + str(winner))
				winnerIDs = [player[0]]
				currRow = player
			else:
				winnerIDs.append( player[0] )
...
def updatePlayoffStats(week, season):
	...
	if( week == "18" ):
		# grab the game data
		cur.execute("select homeScore+awayScore, status from Game where season=" + season + " and weekNumber=18 order by gameTime desc")
		...
		for conf in conferences:
			...
			cur.execute("select userID, PlayoffResult.points, abs(tieBreaker1 - " + str(gameScores[0][0]) + ") as tb1, tieBreaker1, abs(tieBreaker2 - " + str(gameScores[1][0]) + ") as tb2, tieBreaker2, abs(tieBreaker3 - " + str(gameScores[2][0]) + ") as tb3, tieBreaker3, abs(tieBreaker4 - " + str(gameScores[3][0]) + ") as tb4, tieBreaker4 from PlayoffResult join SeasonResult using (userID, season) join Division using (divID) where weekNumber=18 and season=" + season + " and confID=" + str(conf[0]) + " and firstRoundBye='N' order by points desc, tb1 asc, tieBreaker1 asc, tb2 asc, tieBreaker2 asc, tb3 asc, tieBreaker3 asc, tb4 asc, tieBreaker4 asc, userID")
			leaders = cur.fetchall()
			for player in leaders:
				# see if this row is ahead of the current one
				if winnerCount < 10 and (player[1] < currRow[1] or (player[1] == currRow[1] and gameScores[0][1]  == 3 and (player[2] > currRow[2] or (player[2] == currRow[2] and (player[3] > currRow[3] or (player[3] == currRow[3] and (player[4] > currRow[4] or (player[4] == currRow[4] and (player[5] > currRow[5] or (player[5] == currRow[5] and (player[6] > currRow[6] or (player[6] == currRow[6] and (player[7] > currRow[7] or (player[7] == currRow[7] and (player[8] > currRow[8] or (player[8] == currRow[8] and (player[9] > currRow[9]))))))))))))))))):
					winnerCount = winnerCount + len(winnerIDs)
					for winner in winnerIDs:
						cur.execute("update PlayoffResult set advances=if(" + str(winnerCount) + " <= 10, 'Y', 'R') where weekNumber=18 and season=" + season + " and userID=" + str(winner))
					winnerIDs = [player[0]]
					currRow = player
				else:
					winnerIDs.append( player[0] )
		...
		if num[0][0] == 0:
			...
			# add in the needed results
			cur.execute("insert into Pick (userID, gameID, points) select userID, gameID, 1 from PlayoffResult join Game using (weekNumber, season) where weekNumber=19 and prevWeek1>=0 ")
	...
	if( week == "19" ):
		# grab the game data
		cur.execute("select homeScore+awayScore, status from Game where season=" + season + " and weekNumber=19 order by gameTime desc")
		gameScores = cur.fetchall()
		...
		if num[0][0] == 0:
			...
			# add in the needed results
			cur.execute("insert into Pick (userID, gameID, points) select userID, gameID, 1 from PlayoffResult join Game using (weekNumber, season) where weekNumber=20 and prevWeek2>=0 ")
			cur.execute("insert into Pick (userID, gameID, points, type) select userID, gameID, 1, 'winner2Q' from PlayoffResult join Game using (weekNumber, season) where weekNumber=20 and prevWeek2>=0 ")
	...
	if( week == "20" ):
		# grab the game data
		cur.execute("select homeScore+awayScore, homeScore2Q+awayScore2Q, status from Game where season=" + season + " and weekNumber=20 order by gameTime desc")
		gameScores = cur.fetchall()
		...
		if num[0][0] == 0:
			...
			# add in the needed results
			cur.execute("insert into Pick (userID, gameID, points, type) select userID, gameID, 0, 'winner' from PlayoffResult join Game using (weekNumber, season) where weekNumber=22 and prevWeek3>=0 ")
			cur.execute("insert into Pick (userID, gameID, points, type) select userID, gameID, 9, 'winner3Q' from PlayoffResult join Game using (weekNumber, season) where weekNumber=22 and prevWeek3>=0 ")
			cur.execute("insert into Pick (userID, gameID, points, type) select userID, gameID, 8, 'winner2Q' from PlayoffResult join Game using (weekNumber, season) where weekNumber=22 and prevWeek3>=0 ")
			cur.execute("insert into Pick (userID, gameID, points, type) select userID, gameID, 7, 'winner1Q' from PlayoffResult join Game using (weekNumber, season) where weekNumber=22 and prevWeek3>=0 ")
			cur.execute("insert into Pick (userID, gameID, points, type) select userID, gameID, 5, 'passYds' from PlayoffResult join Game using (weekNumber, season) where weekNumber=22 and prevWeek3>=0 ")
			cur.execute("insert into Pick (userID, gameID, points, type) select userID, gameID, 3, 'passYds2Q' from PlayoffResult join Game using (weekNumber, season) where weekNumber=22 and prevWeek3>=0 ")
			cur.execute("insert into Pick (userID, gameID, points, type) select userID, gameID, 5, 'rushYds' from PlayoffResult join Game using (weekNumber, season) where weekNumber=22 and prevWeek3>=0 ")
			cur.execute("insert into Pick (userID, gameID, points, type) select userID, gameID, 3, 'rushYds2Q' from PlayoffResult join Game using (weekNumber, season) where weekNumber=22 and prevWeek3>=0 ")
			cur.execute("insert into Pick (userID, gameID, points, type) select userID, gameID, 2, 'TDs' from PlayoffResult join Game using (weekNumber, season) where weekNumber=22 and prevWeek3>=0 ")
			cur.execute("insert into Pick (userID, gameID, points, type) select userID, gameID, 1, 'TDs2Q' from PlayoffResult join Game using (weekNumber, season) where weekNumber=22 and prevWeek3>=0 ")

	# update super bowl stats
	if( week == "22" ):
		# grab the game score
		cur.execute("select if(homeScore>awayScore, homeTeam, if(homeScore<awayScore, awayTeam, 'TIE')) from Game where season=" + season + " and weekNumber=22 order by gameTime desc")
		leader = cur.fetchall()

		# all we worry about here is whether they picked the correct winner or not
		cur.execute("update PlayoffResult join Game using (season, weekNumber) join Pick using (userID, gameID) set advances=if('" + leader[0][0] + "' = 'TIE', 'R', if(winner='" + leader[0][0] + "', 'Y', 'N')) where weekNumber=22 and type='winner' ")

# define the update function
def updateConsolationStats(season):
	# reset playoff pool stats for this week
	cur.execute("update ConsolationResult join Game as WC1AFC on (WC1AFC.season=ConsolationResult.season and WC1AFC.gameID=(select gameID from Game where season=2014 and weekNumber>17 order by gameID limit 0,1)) join Game as WC2AFC on (WC2AFC.season=ConsolationResult.season and WC2AFC.gameID=(select gameID from Game where season=2014 and weekNumber>17 order by gameID limit 1,1)) join Game as WC1NFC on (WC1NFC.season=ConsolationResult.season and WC1NFC.gameID=(select gameID from Game where season=2014 and weekNumber>17 order by gameID limit 2,1)) join Game as WC2NFC on (WC2NFC.season=ConsolationResult.season and WC2NFC.gameID=(select gameID from Game where season=2014 and weekNumber>17 order by gameID limit 3,1)) join Game as DIV1AFC on (DIV1AFC.season=ConsolationResult.season and DIV1AFC.gameID=(select gameID from Game where season=2014 and weekNumber>17 order by gameID limit 4,1)) join Game as DIV2AFC on (DIV2AFC.season=ConsolationResult.season and DIV2AFC.gameID=(select gameID from Game where season=2014 and weekNumber>17 order by gameID limit 5,1)) join Game as DIV1NFC on (DIV1NFC.season=ConsolationResult.season and DIV1NFC.gameID=(select gameID from Game where season=2014 and weekNumber>17 order by gameID limit 6,1)) join Game as DIV2NFC on (DIV2NFC.season=ConsolationResult.season and DIV2NFC.gameID=(select gameID from Game where season=2014 and weekNumber>17 order by gameID limit 7,1)) join Game as CONFAFC on (CONFAFC.season=ConsolationResult.season and CONFAFC.gameID=(select gameID from Game where season=2014 and weekNumber>17 order by gameID limit 8,1)) join Game as CONFNFC on (CONFNFC.season=ConsolationResult.season and CONFNFC.gameID=(select gameID from Game where season=2014 and weekNumber>17 order by gameID limit 9,1)) join Game as SB on (SB.season=ConsolationResult.season and SB.gameID=(select gameID from Game where season=2014 and weekNumber>17 order by gameID limit 10,1)) set points = if((wc1AFC=WC1AFC.homeTeam and WC1AFC.homeScore>WC1AFC.awayScore) or (wc1AFC=WC1AFC.awayTeam and WC1AFC.awayScore>WC1AFC.homeScore), 1, 0) + if((wc2AFC=WC2AFC.homeTeam and WC2AFC.homeScore>WC2AFC.awayScore) or (wc2AFC=WC2AFC.awayTeam and WC2AFC.awayScore>WC2AFC.homeScore), 1, 0) + if((wc1NFC=WC1NFC.homeTeam and WC1NFC.homeScore>WC1NFC.awayScore) or (wc1NFC=WC1NFC.awayTeam and WC1NFC.awayScore>WC1NFC.homeScore), 1, 0) + if((wc2NFC=WC2NFC.homeTeam and WC2NFC.homeScore>WC2NFC.awayScore) or (wc2NFC=WC2NFC.awayTeam and WC2NFC.awayScore>WC2NFC.homeScore), 1, 0) + if((div1AFC=DIV1AFC.homeTeam and DIV1AFC.homeScore>DIV1AFC.awayScore) or (div1AFC=DIV1AFC.awayTeam and DIV1AFC.awayScore>DIV1AFC.homeScore), 2, 0) + if((div2AFC=DIV2AFC.homeTeam and DIV2AFC.homeScore>DIV2AFC.awayScore) or (div2AFC=DIV2AFC.awayTeam and DIV2AFC.awayScore>DIV2AFC.homeScore), 2, 0) + if((div1NFC=DIV1NFC.homeTeam and DIV1NFC.homeScore>DIV1NFC.awayScore) or (div1NFC=DIV1NFC.awayTeam and DIV1NFC.awayScore>DIV1NFC.homeScore), 2, 0) + if((div2NFC=DIV2NFC.homeTeam and DIV2NFC.homeScore>DIV2NFC.awayScore) or (div2NFC=DIV2NFC.awayTeam and DIV2NFC.awayScore>DIV2NFC.homeScore), 2, 0) + if((confAFC=CONFAFC.homeTeam and CONFAFC.homeScore>CONFAFC.awayScore) or (confAFC=CONFAFC.awayTeam and CONFAFC.awayScore>CONFAFC.homeScore), 4, 0) + if((confNFC=CONFNFC.homeTeam and CONFNFC.homeScore>CONFNFC.awayScore) or (confNFC=CONFNFC.awayTeam and CONFNFC.awayScore>CONFNFC.homeScore), 4, 0) + if((superBowl=SB.homeTeam and SB.homeScore>SB.awayScore) or (superBowl=SB.awayTeam and SB.awayScore>SB.homeScore), 8, 0), picksCorrect=if((wc1AFC=WC1AFC.homeTeam and WC1AFC.homeScore>WC1AFC.awayScore) or (wc1AFC=WC1AFC.awayTeam and WC1AFC.awayScore>WC1AFC.homeScore), 1, 0) + if((wc2AFC=WC2AFC.homeTeam and WC2AFC.homeScore>WC2AFC.awayScore) or (wc2AFC=WC2AFC.awayTeam and WC2AFC.awayScore>WC2AFC.homeScore), 1, 0) + if((wc1NFC=WC1NFC.homeTeam and WC1NFC.homeScore>WC1NFC.awayScore) or (wc1NFC=WC1NFC.awayTeam and WC1NFC.awayScore>WC1NFC.homeScore), 1, 0) + if((wc2NFC=WC2NFC.homeTeam and WC2NFC.homeScore>WC2NFC.awayScore) or (wc2NFC=WC2NFC.awayTeam and WC2NFC.awayScore>WC2NFC.homeScore), 1, 0) + if((div1AFC=DIV1AFC.homeTeam and DIV1AFC.homeScore>DIV1AFC.awayScore) or (div1AFC=DIV1AFC.awayTeam and DIV1AFC.awayScore>DIV1AFC.homeScore), 1, 0) + if((div2AFC=DIV2AFC.homeTeam and DIV2AFC.homeScore>DIV2AFC.awayScore) or (div2AFC=DIV2AFC.awayTeam and DIV2AFC.awayScore>DIV2AFC.homeScore), 1, 0) + if((div1NFC=DIV1NFC.homeTeam and DIV1NFC.homeScore>DIV1NFC.awayScore) or (div1NFC=DIV1NFC.awayTeam and DIV1NFC.awayScore>DIV1NFC.homeScore), 1, 0) + if((div2NFC=DIV2NFC.homeTeam and DIV2NFC.homeScore>DIV2NFC.awayScore) or (div2NFC=DIV2NFC.awayTeam and DIV2NFC.awayScore>DIV2NFC.homeScore), 1, 0) + if((confAFC=CONFAFC.homeTeam and CONFAFC.homeScore>CONFAFC.awayScore) or (confAFC=CONFAFC.awayTeam and CONFAFC.awayScore>CONFAFC.homeScore), 1, 0) + if((confNFC=CONFNFC.homeTeam and CONFNFC.homeScore>CONFNFC.awayScore) or (confNFC=CONFNFC.awayTeam and CONFNFC.awayScore>CONFNFC.homeScore), 1, 0) + if((superBowl=SB.homeTeam and SB.homeScore>SB.awayScore) or (superBowl=SB.awayTeam and SB.awayScore>SB.homeScore), 1, 0) where ConsolationResult.season=" + season)
