    ...
    if( $dumpSet )
    {
      ...
      else if( $_POST["dataDumpID"] == "SPW" && $week >= 18 )
      {
        $query = "select userID, concat(firstName, ' ', lastName) as pName, winner, if(winner='TIE', winner, alias) as alias, type, " .
                 "tieBreaker1, tieBreaker2, tieBreaker3, tieBreaker4, Pick.points as pPts, advances, prevWeek1, prevWeek2, prevWeek3, " .
                 "firstRoundBye, PlayoffResult.points as wPts, SeasonResult.points as sPts, Conference.name as cName, " .
                 "Division.name as dName, weeklyWins, ";
      ...
      else if( $_POST["dataDumpID"] == "CPS" )
      {
        ...
        $dumpResults = runQuery( "select userID, concat(firstName, ' ', lastName) as pName, ConsolationResult.points as cPts, " .
                                 ...
                                 "wc1A.alias as wc1AFCalias, wc2A.alias as wc2AFCalias, wc1N.alias as wc1NFCalias, wc2N.alias as wc2NFCalias, " .
                                 "d1A.alias as div1AFCalias, d2A.alias as div2AFCalias, d1N.alias as div1NFCalias, d2N.alias as div2NFCalias, " .
                                 "cA.alias as confAFCalias, cN.alias as confNFCalias, sb.alias as superBowlalias, " .
                                 ...
                                 "left join Team as wc1N on (wc1NFC=wc1N.teamID) left join Team as wc2N on (wc2NFC=wc2N.teamID) " .
    ...
    if( isset($dumpResults) )
    {
      ...
      if( $_POST["dataDumpID"] == "SPW" && $_POST["dataDumpWeek"] < 18 )
      {
        ...
        $games = runQuery( "select concat(aT.alias, '@', hT.alias) as info from Game join Team as aT on (awayTeam=aT.teamID) " .
                           "join Team as hT on (homeTeam=hT.teamID) where weekNumber=" . $week . " and season=" .
      ...
      // consolation pool
        ...
        while( ($thisRow = mysqli_fetch_assoc( $dumpResults )) != null )
        {
          echo "      <span>" . $thisRow["pName"] . "," . $thisRow["wc1AFCalias"] . "," . $thisRow["wc2AFCalias"] . "," .
               $thisRow["wc1NFCalias"] . "," . $thisRow["wc2NFCalias"] . "," . $thisRow["div1AFCalias"] . "," . $thisRow["div2AFCalias"] .
